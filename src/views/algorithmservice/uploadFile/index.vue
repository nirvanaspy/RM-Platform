        ...
        <div style="height:42px">
          <div v-show="reedit">
            ...
            <uploader-btn :directory="true"
                          :single="false">选择文件</uploader-btn>
          </div>
          ...
          <i class="el-icon-close closeUpload"
             ...
             @click="hiddenUpload" />
          <div ref="button_upload"
               style="width:90px;position: absolute;left: 610px;top: 0;transition: all 0.5s;">
            <el-button type="primary"
                       icon="el-icon-upload"
                       style="border-radius:10px"
                       @click="realUpload">上传</el-button>
          </div>
        </div>
...
import { postMerge, algorithmAndServerId, realupload } from '@/api/upload'
...
export default {
  ...
  watch: {
    ...
    reedit() {
      ...
      this.$refs.button_upload.style.left = '610px'
    }
  },
  methods: {
    realUpload() {
      if (this.reedit || this.getfileList().length === 0) {
        this.$message({
          message: `请先选择文件`,
          type: 'info'
        })
        ...
        this.$refs.button_upload.style.left = '0px'
        ...
        algorithmAndServerId(qs.stringify(obj)).then((data) => {
    ...
    commitUpload() {
      ...
      // }
      console.log('合并')
      realupload(this.resourceid, fileobj, { 'parentNodeId': this.resourceid }).then(() => {
        this.$notify({
          title: '成功',
          message: `uploader完成`,
          type: 'success',
...
.uploadWrapper .uploader-file-icon[icon="document"] {
  background-image: url("../../../assets/images/word文档.png");
  background-size: contain;
  background-repeat: no-repeat;
}
.uploadWrapper .uploader-file-icon[icon="unknown"] {
  background-image: url("../../../assets/images/文件.png");
  background-size: contain;
  background-repeat: no-repeat;
}
.uploadWrapper .uploader-file-icon[icon="image"] {
  background-image: url("../../../assets/images/图片.png");
  background-size: contain;
  background-repeat: no-repeat;
}
.uploadWrapper .uploader-file-icon[icon="zip"] {
  background-image: url("../../../assets/images/压缩包.png");
  background-size: contain;
  background-repeat: no-repeat;
}
