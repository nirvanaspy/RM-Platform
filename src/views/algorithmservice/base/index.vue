...
import { algorithmAndServerlist, algorithmAndServerconditon } from '@/api/fileSearch'
import { algorithmAndServer } from '@/api/download'
...
export default {
  ...
  methods: {
    getList() {
      ...
      obj.page = obj.page - 1
      algorithmAndServerlist(obj).then((res) => {
    ...
    handleFilter() {
      ...
      algorithmAndServerconditon(data).then((data) => {
    ...
    handleModifyStatus(row) {
      algorithmAndServer(this.user_id, row.id).then((data) => {
        console.log(data)
