  ...
  <div class="uploadWrapper">
    ...
    <transition name="fade">
      <uploader v-if="readyUpload"
        ...
        <uploader-unsupport />
        <uploader-btn :directory="false"> <i class="el-icon-upload" />选择文件

        </uploader-btn>
        <i class="el-icon-close closeUpload"
           style="float: right;width: 32px;height: 32px; margin-top: 10px;margin-right: 10px;text-align: center;"
           @click="hiddenUpload" />
...
import { postMerge } from '@/api/upload'
...
export default {
  ...
  methods: {
    ...
    commitUpload(file) {
      // 文件上传成功后添加描述信息
      commitdescrition(qs.stringify(...this.information)).then(() => {
        this.$notify({
          title: '成功',
          message: `文件${file.name}uploader完成`,
          type: 'success',
          position: 'bottom-right'
        })
      }).catch(() => {
        this.$notify({
          title: '失败',
          message: `文件${file.name}uploader失败，检查网络重新上传`,
          type: 'error',
          position: 'bottom-right'
        })
      })
